...
def get_db_connection():
    return pymysql.connect(
        host="localhost",
        user="root",
        password=os.getenv('DB_PASSWORD'),  # Utiliser la variable d'environnement pour le mot de passe
        database="bechdel",
...
# 1) se rendre dans le répertoire via le terminal, 
# 2) activer l'environnement virtuel (source venv/bin/activate), 
# 3) exécuter le script: python app2.py
# 4) vérifier le résultat via le navigateur: http://localhost:5001/movies/1, ou http://localhost:5001/movies